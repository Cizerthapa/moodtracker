...
class _AdminPanelScreenState extends State<AdminPanelScreen>
    ...
    return Theme(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          centerTitle: false,
          leading: IconButton(
            icon: const Icon(Icons.arrow_back_ios_new_rounded, color: _textSecondary),
          ...
          title: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.admin_panel_settings_rounded, color: _danger, size: 20.r),
              ),
              SizedBox(width: 12.w),
              Column(
          ...
          bottom: TabBar(
            ...
            labelStyle: GoogleFonts.outfit(fontWeight: FontWeight.w600, fontSize: 13.sp),
            tabs: const [
              Tab(icon: Icon(Icons.people_alt_rounded, size: 18), text: 'Users'),
              Tab(icon: Icon(Icons.photo_library_rounded, size: 18), text: 'Memories'),
  ...
  Widget _buildStatsBar() {
    return Container(
      ...
      child: Row(
        children: [
          _StatChip(label: 'Users', value: _stats['users'], loading: _loadingStats),
          SizedBox(width: 12.w),
          _StatChip(label: 'Memories', value: _stats['memories'], loading: _loadingStats),
          SizedBox(width: 12.w),
          _StatChip(label: 'Journals', value: _stats['journals'], loading: _loadingStats),
          const Spacer(),
          IconButton(
            icon: Icon(Icons.refresh_rounded, color: _textSecondary, size: 18.r),
            onPressed: () {
...
class _StatChip extends StatelessWidget {
  ...
  final int? value;
  final bool loading;
  const _StatChip({required this.label, required this.value, required this.loading});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          loading
              ? SizedBox(width: 12.r, height: 12.r, child: const CircularProgressIndicator(strokeWidth: 1.5, color: _accent))
              : Text(
                  '${value ?? 0}',
                  style: GoogleFonts.jetBrainsMono(color: _accent, fontSize: 13.sp, fontWeight: FontWeight.w700),
                ),
          SizedBox(width: 6.w),
          Text(label, style: GoogleFonts.outfit(color: _textSecondary, fontSize: 11.sp)),
        ],
      ),
    );
  }
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return Center(
            child: Text('No users found.', style: GoogleFonts.outfit(color: _textSecondary)),
          );
        }
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            SizedBox(width: 8.w),
            Text('Delete User', style: GoogleFonts.outfit(color: _textPrimary, fontWeight: FontWeight.w700)),
          ],
        ),
        content: RichText(
          text: TextSpan(
            ...
            children: [
              ...
              TextSpan(text: email, style: const TextStyle(color: _textPrimary, fontWeight: FontWeight.bold)),
              const TextSpan(text: ' and all their data (memories, journals, etc.).\n\nThis action cannot be undone.'),
            ],
          ),
        ),
        actions: [
            ...
            onPressed: () async {
              ...
              if (context.mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('User $email deleted.'), backgroundColor: _danger),
                );
              }
            },
...
class _MemoriesTabState extends State<_MemoriesTab> {
  ...
  Future<void> _loadMemories(String uid) async {
    setState(() { _loadingMemories = true; _memories = []; });
    final mems = await widget.repo.getUserMemories(uid);
    if (mounted) setState(() { _memories = mems; _loadingMemories = false; });
  }
      ...
      children: [
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onTap: () {
                  setState(() { _selectedUid = uid; _selectedEmail = email; });
                  _loadMemories(uid);
                },
                child: AnimatedContainer(
                  ...
                  child: Text(
                    email,
                    style: GoogleFonts.outfit(color: isSelected ? _accent : _textSecondary, fontSize: 12.sp, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w400),
                  ),
                ),
              );
            },
        ...
        Expanded(
          child: _selectedUid == null
              ? Center(child: Text('Select a user above to view their memories.', style: GoogleFonts.outfit(color: _textSecondary, fontSize: 13.sp), textAlign: TextAlign.center))
              : _loadingMemories
                  ? const Center(child: CircularProgressIndicator(color: _accent))
                  : _memories.isEmpty
                      ? Center(child: Text('No memories found for $_selectedEmail.', style: GoogleFonts.outfit(color: _textSecondary)))
                      : ListView.builder(
                          padding: EdgeInsets.all(16.r),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: Row(
                                children: [
                                  Icon(
                                    mem.isUnique ? Icons.star_rounded : Icons.favorite_rounded,
                                    color: mem.isUnique ? const Color(0xFFD29922) : _danger,
                                    size: 18.r,
                                  ),
                                  SizedBox(width: 12.w),
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(mem.title, style: GoogleFonts.outfit(color: _textPrimary, fontWeight: FontWeight.w600, fontSize: 14.sp)),
                                        Text(mem.description, style: GoogleFonts.outfit(color: _textSecondary, fontSize: 11.sp), maxLines: 1, overflow: TextOverflow.ellipsis),
                                        if (mem.memoryDate != null)
                                          Text(
                                            '${mem.memoryDate!.day}/${mem.memoryDate!.month}/${mem.memoryDate!.year}',
                                            style: GoogleFonts.jetBrainsMono(color: _textSecondary, fontSize: 10.sp),
                                          ),
                                      ],
                                    ),
                                  ),
                                  IconButton(
                                    icon: Icon(Icons.delete_outline_rounded, color: _danger, size: 18.r),
                                    onPressed: mem.id != null ? () => _confirmDeleteMemory(context, mem) : null,
                                  ),
                                ],
                              ),
                            );
  ...
  void _confirmDeleteMemory(BuildContext context, MemoryModel mem) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF21262D),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.r)),
        title: Text('Delete Memory', style: GoogleFonts.outfit(color: _textPrimary, fontWeight: FontWeight.w700)),
        content: Text(
          'Delete "${mem.title}"? This cannot be undone.',
          style: GoogleFonts.outfit(color: _textSecondary, fontSize: 13.sp),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Cancel', style: GoogleFonts.outfit(color: _textSecondary))),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
...
class _BroadcastTabState extends State<_BroadcastTab> {
  ...
  final Map<String, Map<String, dynamic>> _types = {
    ...
    'warning': {'label': 'Warning', 'color': Color(0xFFD29922), 'icon': Icons.warning_rounded},
    'success': {'label': 'Success', 'color': Color(0xFF3FB950), 'icon': Icons.check_circle_rounded},
    'danger': {'label': 'Danger', 'color': Color(0xFFFF7B72), 'icon': Icons.error_rounded},
  };
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          // ── Current Broadcast ──────────────────────────────────
          Text('Current Active Broadcast', style: GoogleFonts.outfit(color: _textPrimary, fontWeight: FontWeight.w700, fontSize: 16.sp)),
          ...
          StreamBuilder(
            ...
            builder: (context, snapshot) {
              if (!snapshot.hasData || !snapshot.data!.exists) {
                return Container(
                  padding: EdgeInsets.all(16.r),
                  decoration: BoxDecoration(color: _card, borderRadius: BorderRadius.circular(12.r), border: Border.all(color: _border)),
                  child: Text('No active broadcast.', style: GoogleFonts.outfit(color: _textSecondary, fontSize: 13.sp)),
                );
              }
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 12.w),
                    Expanded(child: Text(msg, style: GoogleFonts.outfit(color: _textPrimary, fontSize: 13.sp))),
                    IconButton(
                      icon: Icon(Icons.close_rounded, color: _danger, size: 18.r),
          ...
          // ── Send New Broadcast ─────────────────────────────────
          Text('Send New Broadcast', style: GoogleFonts.outfit(color: _textPrimary, fontWeight: FontWeight.w700, fontSize: 16.sp)),
          SizedBox(height: 12.h),
            ...
            children: _types.entries.map((entry) {
              ...
              return Expanded(
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    duration: const Duration(milliseconds: 200),
                    margin: EdgeInsets.only(right: entry.key == _types.keys.last ? 0 : 8.w),
                    ...
                    child: Column(
                      children: [
                        Icon(entry.value['icon'] as IconData, color: color, size: 18.r),
                        SizedBox(height: 4.h),
                        Text(entry.value['label'] as String, style: GoogleFonts.outfit(color: isSelected ? color : _textSecondary, fontSize: 10.sp)),
                      ],
                    ),
                  ),
                ),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle: GoogleFonts.outfit(color: _textSecondary, fontSize: 13.sp),
              filled: true,
              fillColor: _card,
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r), borderSide: BorderSide(color: _border)),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r), borderSide: BorderSide(color: _border)),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_sending ? 'Sending...' : 'Send Broadcast', style: GoogleFonts.outfit(fontWeight: FontWeight.w700, fontSize: 15.sp)),
            ),
          ),
        ],
      ),
  ...
  Future<void> _sendBroadcast() async {
    ...
    if (msg.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Please enter a message.', style: GoogleFonts.outfit()), backgroundColor: _danger));
      return;
    }
    setState(() => _sending = true);
    await widget.repo.sendBroadcast(msg, type: _selectedType);
    if (mounted) {
      setState(() { _sending = false; _messageController.clear(); });
      HapticFeedback.lightImpact();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Broadcast sent!', style: GoogleFonts.outfit()), backgroundColor: _success));
    }
  }
}
